...
if ($mform->is_cancelled()) {
    ...
    if ($isadding) {
       ...
       if (!empty($data->hostname)) {
           ...
           $data->hostname = $data->hostname.'.'.$_SERVER["SERVER_NAME"];
       
            $dnsHostname = $data->hostname;
                // Test if string contains the word 
                if(strpos($data->hostname, $_SERVER["SERVER_NAME"]) !== false){
                    $dnsHostname = substr($data->hostname, 0, strpos($data->hostname, '.'));
                }
                
            updateDns("A", $dnsHostname,$_SERVER["SERVER_ADDR"],strtolower($_SERVER["SERVER_NAME"]));
             
             createSubDomain($data->hostname,strtolower($_SERVER["SERVER_NAME"]),getServerAddresWithoutHome(),$_SERVER["SERVER_ADDR"]);
        ...
        if(strtolower($oldcompany->hostname) != strtolower($data->hostname)){
            ...
            $dnsDeleteHostname = $oldcompany->hostname;
                // Test if string contains the word 
                if(strpos($oldcompany->hostname, $_SERVER["SERVER_NAME"]) !== false){
                    $dnsDeleteHostname = substr($oldcompany->hostname, 0, strpos($oldcompany->hostname, '.'));
                }
            
            deleteDnsRecord("A", $dnsDeleteHostname,$_SERVER["SERVER_ADDR"],strtolower($_SERVER["SERVER_NAME"]));
            
            deleteSubDomain($oldcompany->hostname,strtolower($_SERVER["SERVER_NAME"]),getServerAddresWithoutHome(),$_SERVER["SERVER_ADDR"]);
