...
class block_iomad_company_admin extends block_base {
    ...
    public function get_content() {
        global $OUTPUT, $CFG, $SESSION, $USER, $DB;
        ...
        $role = key($roles);
        $userRoleName = $roles[$role]->shortname;
        ...
        $shouldShowAdminTabs = false;
        
        if ($userRoleName == 'companymanager'){
            $companyId = iomad::get_my_companyid(context_system::instance(), false);
            $companySubscription = $DB->get_record_sql("SELECT * FROM {subscriptions} WHERE status = 'Paused' AND company_id = $companyId;");
            $shouldShowAdminTabs = empty($companySubscription);
            
            
        }else if ($userRoleName == null){
            $shouldShowAdminTabs = true;
        }
        // Build tabs.
        $tabs = [];
        $panes = [];
        ...
        if (iomad::has_capability('block/iomad_company_admin:usermanagement_view', $context) && $shouldShowAdminTabs) {
            $tabs[] = [
                'category' => 'UserAdmin',
                'icon' => 'fa-user',
                'selected' => $selected,
        ...
        if (iomad::has_capability('block/iomad_company_admin:coursemanagement_view', $context) && $shouldShowAdminTabs) {
            $tabs[] = [
        ...
        if (iomad::has_capability('block/iomad_company_admin:licensemanagement_view', $context) && $shouldShowAdminTabs) {
            $tabs[] = [
        ...
        if (iomad::has_capability('block/iomad_company_admin:competencymanagement_view', $context) && $shouldShowAdminTabs) {
            $tabs[] = [
        ...
        if (iomad::has_capability('block/iomad_commerce:admin_view', $context) && $shouldShowAdminTabs) {
            $tabs[] = [
        ...
        if (iomad::has_capability('block/iomad_microlearning:view', $context) && $shouldShowAdminTabs) {
            $tabs[] = [
        ...
        if (iomad::has_capability('block/iomad_reports:view', $context) && $shouldShowAdminTabs) {
            $tabs[] = [
                'category' => 'Reports',
                'icon' => 'fa-bar-chart-o',
                'selected' => $selected,
        ...
        foreach ($menus as $key => $menu) {
            ...
            if (!empty($menu['name'])) {
             ...
             //var_dump($menu['name']);
            if (($menu['name'] == 'Edit company' || $menu['name'] == 'Manage departments' ||
                 $menu['name'] == 'Optional profiles' || $menu['name'] == 'Email templates') && !$shouldShowAdminTabs){
                continue;
            }
